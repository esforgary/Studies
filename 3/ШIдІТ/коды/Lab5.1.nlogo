...
figures-own [queen-position domain possible-steps step-performed?]
edges-own [weight]
...
to set-queen-positions
  let other-queens other figures with [breed = queens]
  let positions []
  foreach x-positions [
    [i] ->
    foreach y-positions [
      [j] ->
        set positions fput list i j positions
    ]
  ]
  let filtered-positions []
  foreach other-queens [
    q ->
    let qp [queen-position] of q
    ifelse qp != nobody [
      set filtered-positions filter [p -> not member? p qp] filtered-positions
    ] [
      set filtered-positions positions
    ]
  ]
  set domain filtered-positions
end
@#$#@#$#@
GRAPHICS-WINDOW
276
